...
元件式設計其實並不困難，但當資料需要透過跨元件傳遞時，通常都會讓初學者有點搞不清楚使用的時機與方；什麼是跨元件溝通呢？以常用的社群應用程式作為例子好了，在使用某 `F` 開頭的社群應用程式中，當收到新私訊時選單上會出現紅色圈圈提醒，打開私訊小視窗的後該圈圈就會自動消失，這就是基本的跨元件溝通，右下角的小視窗和選單上的 icon 基本上個別的元件，他們之間的資料傳遞需要透過某種機制的傳遞才能達到狀態 `（state）` 的一致性。
...
> 這裡會細說設計 Vue 應用程式中狀態和元件的組成，如果你只是想看跨元件溝通的方式可以直接跳到[這](#%e8%b7%a8%e5%85%83%e4%bb%b6%e6%ba%9d%e9%80%9a%e7%9a%84%e6%96%b9%e5%bc%8f%e6%9c%89-n-%e7%a8%ae)。
...
要怎麼拆成`元件`呢？ 我們可以將 `Todo App` 裡的功能都歸類為元件，也就是剛剛說的 `input#newTodo` 和 `li.todo[]` 。拆成元件後 code 可以是：
...
Vue.component("Todo", {
  ...
  methods: {
    deleteTodo() {
      bus.$emit("delete", { id: this.todo.id }); // 原本的 this 改成宣告的bus變數
    }
  }
});
```
...
Vuex 借鑒了 Flex 和 Redux 的理念，以 `store` 存取資料的概念，它和EventBus的集合巴士相似，用 `倉庫` 去理解他或許會比較好懂，這倉庫裡包含了所有的狀態的商業邏輯，把需要被監聽的狀態，全權交給 `store` 處理。`Vuex` 主要分為三個概念：
...
Vuex 是個比較進階的概念，包含 `setters` 與 `getters`，我在這裡就不太深入敘述，有興趣的人可以查看[官方文件](https://vuex.vuejs.org/) 或是 [此篇簡介](https://www.itread01.com/content/1544948592.html)。
...
const store = new Vuex.Store({
  ...
  actions: { // 欲提交的事件
    ...
    setTodo({ commit, state }, todo) { // 非同步或多重commit
      ...
      let getOriginalTodos = [...state.todos];
      commit('SAVING_START');

      return fetch('/saveTodo', {body: JSON.Stringify(todo), method: 'POST'})
        .then(res => res.json())
        .then(data => {
          if (data.state) commit('SET_TODO', todo);
        })
        .catch(error => {
          commit('SAVING_FAILED', {todos: getOriginalTodos, error});
        })
        .finally(() => commit('SAVING_END'));

    },
    deleteTodo({ commit, state }, id) { // 可帶條件
...
new Vue({
  ...
  methods: {
    deleteTodo(id) {
      store.dispatch('deleteTodo', id); // 就直接呼叫某個action即可
    },
    saveTodo(todo) {
...
```
在 `#app` 裡比較重要的改變會是狀態的取得方式，我們改成 `computed` 的方式監聽 `store` 裡的 `todos` 狀態，在任何元件內都可以呼叫store的 `dispatch` 函式，這樣可以把所有跟狀態相關的函式組織在倉庫內，邏輯切分的清楚。
...
<script>
  ...
  new Vue({
    ...
    methods: {
      triggerChild(e) {
        this.$children[0].value = e.target.value; // 設定第一個字層的狀態 
      }
    }
  });
</script>
...
2. `$children` 的用法比較特別：從父層裡設定某個 `$children` 的值。此案例裡，在頁面上input綁定了 `newTodo` 狀態， `@change` 事件發生的時候會觸發 `this.$children[0].value = e.target.value;`，也就是將子層內的某個狀態對應到自己本身。

這個案例實際上單純只是為了示範 `$parent` 和 `$children` 的使用方式，更好的做法是將 `newTodo` 雙向綁定就可以達到想要的效果了。

> 💡 `this.$children` 是陣列 `(Array)` 型別，依據目前view中渲染的數量而定，它是動態的值，使用的時候要特別小心，當子階層含有非常複雜的結構時， 就會無法正確地取得`$children` 的 index 值，這是 `$children` 的缺陷。
...
> 💡 依然還是可以使用 props 和 value binding 的方式傳值， $ref 單純是取得該元件的實例。
...
Vue.component('fake-user', {
  created() {
    console.log(this.$attrs); // { name: 'kangw3n', age: '30', title="custom-title" }
  }
});
...
Vue.component('fake-user', {
  inheritAttrs: false, // 設定 false 會導致在dom渲染上把屬性隱藏
  created() {
    console.log(this.$attrs); // { name: 'kangw3n', age: '30', title="custom-title" }
  }
});

<!-- inheritAttrs = false 後實際頁面渲染的結果 -->
<div></div>
```
...
Vue.component('fake-user', {
  props: ['age'],
  created() {
    console.log(this.$attrs); // { name: 'kangw3n'}
  }
});
```
...
Vue.component('fake-user', {
  template: '<div><child v-bind="$attrs"></child></div>', // 這裡是重點
  props: ['age'],
  created() {
...
Vue.component('fake-user', {
  template: '<div><child v-on="inputListeners"></child></div>',
  computed: {
    inputListeners() {
      var vm = this;
      return {
        ...this.$listeners,
        custom1(event) {
          console.log('我在這裡攔截custom1並不讓他往上傳'); // 把第二層的事件攔截
...
從 `$listeners` 就可以把所有子元件相關的事件都往上拋，中間可以依據條件的設定做攔截，修改之間的溝通協定，比起要一層一層傳來的簡單多了，也不需要開啟 Event Bus 去佔用多餘的實例。
