...
這時候的 `a` 會是誰呢？ 如果你說 `1` 那你就對啦。如果調一下位置呢？
...
這時候有人會說應該是函式 `a` 吧？因為他在變數 `a` 的後面，對不起你錯了，結果還是 `1`， 原因一樣跟提升有關，大概可以透過這樣的方式解釋。
...
console.log(a); // << console 掉在這

b = 3;
```

> 👉 在spec的規範是，`函式宣告` 會提早被提升，接著是變數；`函式宣告` 則比 `變數宣告` 有更大的優先權，但小於 `變數賦值`，這裡指的優先權我把它定義為：`執行的優先權`。

> ✍ 依據這規範，或許變數提升的排序並無前後關系，決定權在於引擎的導入方式。但在這裡我會依據上面分解圖的理解去解釋提升。

以上的兩個案例，基本上跟上面的分解圖是一樣的，就算順序上有差，`var` 宣告的都會被推上去，而賦值大於函式宣告。
...
依據上面的說法，大家可以猜猜這四個 log 會是什麼結果呢？來看看解析圖吧：

```javascript 
var a; // 1. 變數宣告提升上來沒問題
function a(){console.log('1')} // 2. 函式宣告排序 1
function a(){console.log('2')} // 3. 函式宣告排序 2
function a(){console.log('3')} // 4. 函式宣告排序 3
console.log(a); // 5. 落在這點就上目前的排序，這時候就是 fn(3)
console.log(a); // 6. 落在這點就上目前的排序，這時候就是 fn(3)
a = 4; // 7. 賦值產生
console.log(a); // 8. 此時 a 變 4了
a = 5; // 9. 修改產生
console.log(a); // 10. 此時 a 變 5了
```


其實到這裡就寫完了，提升這個特性也不是JavaScript奇怪的部分，但是每次在回想起來的時候都覺得它具有滿滿的陷阱，所以以後還是不要用 `var` 會比較好，而盡量都將變數給予個預設值，就可以避免排序導致理解的誤會了，因為賦值的權限大於函式。
...
console.log(i);
```
這題應該蠻簡單的，答案就是順序的 `0, 1, 2`，然後因為用 `var` 宣告的關係，第二個log因被提升到上一層而變成`3`，解析圖：
...
猜猜看會有什麼反應，其實此時 `var` 跟 `let` 宣告的 `i` 已經不是同一個了，而因為進入到 for 範疇裡形成了 block scope，所以結果會是 `Cannot access 'i' before initialization`。 解析圖：
...
> 🙏 最後想聲明的是，以上所提出的解析圖都是我個人的立場喲，自己覺得能夠快速了解提升概念而產生的，未必是瀏覽器引擎採用的方式，若有概念錯誤的地方也請各位高手們提出正確的解析。
